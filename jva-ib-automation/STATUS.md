...
| `utils.gs` | 会社メール検索、Drive URL パース、フォーム値取得、フォーム選択肢同期 | ✅ |
...
学生フォームの "Company Name" 質問をプルダウン化し、マスターシートの会社名と自動同期する機能。
手打ちによるマッチングエラーを防ぐ。

### セットアップ手順（一回限り）

1. **Googleフォームを編集:** "Company Name" 質問のタイプを「テキスト入力」→「プルダウン（リスト）」に変更
2. **スクリプトプロパティ追加:** `STUDENT_FORM_ID` に学生応募フォームのID（URL `/d/XXXX/` 部分）を設定
3. **初回同期:** Apps Scriptエディタで `syncFormCompanyChoices()` を手動実行
4. **トリガー設定:** `syncFormCompanyChoices` に時間ベーストリガーを追加（毎時 or 毎日推奨）

### 動作仕様

- マスターシートのF列（Company Name）を読み込み、重複排除・ソートして選択肢を更新
- 会社数が0件の場合は誤クリアを防ぐためスキップ
- フォームに "Company Name" 質問がない場合はエラーをthrow（タイトル変更への気づき）
