...
const TIMEOUT = parseInt(process.env.AGENT_BROWSER_TIMEOUT ?? "30000", 10);

function runAgentBrowser(args: string[]): string {
  try {
    return execFileSync("agent-browser", ["--profile", PROFILE, ...args], {
      timeout: TIMEOUT,
      encoding: "utf8",
...
// Navigate to URL — CLI: open (alias navigate); uses Chrome profile for LinkedIn session
server.tool(
  "navigate",
  "Navigate to a URL. Uses the saved Chrome profile so LinkedIn sessions persist.",
  { url: z.string().min(1).describe("URL to open (https://... or example.com)") },
  async ({ url }) => {
    const out = runAgentBrowser(["open", url]);
...
server.tool(
  "snapshot",
  "Return the accessibility tree of the current page with element refs (@e1, @e2, ...). Use refs for click/fill commands.",
  {},
  async () => {
    const out = runAgentBrowser(["snapshot"]);
...
server.tool(
  "click",
  "Click an element. Pass a ref from snapshot (e.g. @e3) or a CSS selector.",
  { ref: z.string().describe("Element ref (@e3) or CSS selector") },
  async ({ ref }) => {
    const out = runAgentBrowser(["click", ref]);
...
server.tool(
  "fill",
  "Clear a field and fill it with text. Pass a ref from snapshot (e.g. @e5) or CSS selector.",
  {
    ref: z.string().describe("Element ref (@e5) or CSS selector"),
    text: z.string().describe("Text to fill in"),
  },
  async ({ ref, text }) => {
    const out = runAgentBrowser(["fill", ref, text]);
...
server.tool(
  "press",
  "Press a keyboard key. Use 'Enter' to submit forms or DMs.",
  { key: z.string().describe("Key name e.g. Enter, Tab, Escape, Control+a") },
  async ({ key }) => {
    const out = runAgentBrowser(["press", key]);
...
server.tool(
  "wait",
  "Wait for a number of milliseconds (for slow pages) or for an element selector to appear.",
  { ms: z.number().int().min(100).max(10000).describe("Milliseconds to wait") },
  async ({ ms }) => {
    const out = runAgentBrowser(["wait", String(ms)]);
...
server.tool(
  "screenshot",
  "Take a screenshot of the current page for debugging. Returns the saved file path.",
  {},
  async () => {
    const path = `/tmp/agent-browser-${Date.now()}.png`;
    runAgentBrowser(["screenshot", path]);
...
server.tool(
  "get_text",
  "Get the text content of an element or the entire page body.",
  {
    ref: z
      .string()
      .optional()
  ...
  async ({ ref }) => {
    ...
    const out = runAgentBrowser(["get", "text", target]);
    return { content: [{ type: "text", text: out }] };
  }
);
