...
### Step 3: バックログ更新（Automatic・最優先）

**セッション中に言及されたタスクをバックログに反映する。** これを行ってから仕分け処理に進む。
...
|-----------|----------------|
| 「完了した」「やった」「できた」「終わった」 | `[ ]` → `[x]` |
| 「もうやらなくていい」「キャンセル」「なくなった」「はいい」 | 行を削除 |
| 「追加して」「やりたい」「必要になった」 | 適切な象限に `[ ]` 追加 |
| 象限変更（「急がなくなった」「緊急になった」）| 象限を移動 |

**更新対象ファイル（タスクの所属に応じて）:**
- JVA業務 → `company_jva/secretary/backlog.md`
- 個人・横断 → `company_personal/tasks/backlog.md`

**ルール:**
- バックログ更新はユーザー確認不要。即時実行。
- セッション冒頭での「完了報告」も確実に反映する
- `/evening` が未実行でも /fin_s でバックログを最新化する

---

### Step 4（旧Step3）: 自動仕分け & ファイル更新（Automatic）
...
| 夜の締め・進捗記録 | `/evening` |
| コード/深い作業のセッション終了 | `/fin_s`（このスキル）|

> `/fin_s` はナレッジの仕分け・蓄積が主目的。日常的なタスク管理は `/secretary` に任せる。
